...
// --- Core game state and level tracking ---
// These global variables keep track of the current screen, level
...
// Audio state is initialized during preload and enabled after
// the first user interaction to satisfy browser autoplay restrictions.
...
// Names, power goals, and intro text are defined here for each level.
let levelNames = [
...
// This ensures the music and sound effects are available at level start.
function preload() {
  soundFormats('mp3');
...
// Initialize the player, sphere, and audio handlers here.
function setup() {
...
// This covers both mouse and touch events for mobile and desktop.
...
// Thelnar handles input, physics integration, collisions, and rendering.
class Thelnar {
  constructor(x, y) {
    this.startX = x;
    this.startY = y;
  ...
  // The character is drawn at its current physics position.
  draw() {
...
// The sphere changes physics behavior when the player is nearby.
class Sphere {
...
// Routing is based on the current gameState.
function draw() {
  playMusicForState();
...
// Draw the title screen with game instructions and start prompt
function drawTitle() {
  fill(255);
  textAlign(CENTER);
  textSize(52);
...
// This prepares the level layout, sphere behavior, and player reset.
...
// This resets the player, returns to the title screen, and starts level 1 on next play.
...
// This controls how gravity, speed, and jump force change as difficulty increases.
...
// Draw the level environment, including background, ground, platforms, and HUD
...
function drawWinScreen() {
  ...
  textSize(22);
  text('Press N for next level.', width / 2, height / 2 + 20);
}

// Draw the lose screen shown when the player falls too far or fails a level
...
// Handle global keyboard input for title, win, and lose screens
function keyPressed() {
  enableAudio();
